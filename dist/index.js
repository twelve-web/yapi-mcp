...
server.tool("yapi_get_interfaces", {
    url: z
        .string()
        .describe("YApi分类页面URL，格式如：https://xxxxx.com/project/810/interface/api/cat_2783，用于获取该分类下的所有接口列表，需要拿到ID通过调用yapi_get_interface_detail获取接口详情。"),
}, async ({ url }) => {
    try {
        // 参数验证
        ...
        let result = `📋 分类ID ${catId} 的接口列表 (共${data.data?.total || 0}个):\n\n`;
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `❌ 处理过程中发生错误：${error instanceof Error ? error.message : "未知错误"}`,
                },
            ],
        };
    }
...
// YApi接口详情获取工具
server.tool("yapi_get_interface_detail", {
    id: z
        .string()
        .describe("接口ID，来自接口列表中的_id字段，或者来自直接传入的id用于获取该接口的详情。"),
}, async ({ id }) => {
    try {
        // 参数验证
